...
class MonkSignUpScreen extends GetView<AuthController> {
  const MonkSignUpScreen({super.key});
  ...
  Future<void> _submit(BuildContext context) async {
    ...
    final monasteryName = controller.monasteryNameController.text.trim();
    final monasteryAddress = controller.monasteryAddressController.text.trim();
    ...
    if (monasteryName.isEmpty) {
      Get.snackbar('Required', 'Monastery name is required');
      return;
    }

    if (monasteryAddress.isEmpty) {
      Get.snackbar('Required', 'Monastery address is required');
      return;
    }

    final success = await controller.signupMonk();
    ...
    return AuthPageScaffold(
      child: Column(
        ...
        children: [
          ...
          const AuthSectionTitle('Monastery Information').slideIn(
            delay: 520.ms,
            begin: Offset(0, 0.10),
            fadeBegin: 0,
          ),
          ...
          const AuthFieldLabel('Monastery Name', required: true).slideIn(
            delay: 560.ms,
            begin: Offset(0, 0.10),
            fadeBegin: 0,
          ),
          ...
          CustomInputTextField(
            controller: controller.monasteryNameController,
            textStyle: const TextStyle(
              fontSize: 16,
              color: Colors.black,
            ),
            hintText: 'Enter your monastery name',
            hintStyle: const TextStyle(
              fontSize: 16,
              color: AuthColors.hint,
              fontWeight: FontWeight.w400,
          ...
          const AuthFieldLabel('Monastery Address', required: true).slideIn(
            delay: 630.ms,
            begin: Offset(0, 0.10),
            fadeBegin: 0,
          ),
          ...
          CustomInputTextField(
            controller: controller.monasteryAddressController,
            maxLines: 3,
            textStyle: const TextStyle(
              fontSize: 16,
              color: Colors.black,
            ),
            hintText: 'Enter your monastery address',
            hintStyle: const TextStyle(
              fontSize: 16,
              color: AuthColors.hint,
              fontWeight: FontWeight.w400,
